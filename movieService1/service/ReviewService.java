...
public class ReviewService {
    ...
    public void add_review(String userName, String movieName, int rating) throws Exception {
        ...
        reviewModel review = new reviewModel(user.getName(), movie.getName(), rating * weight, // to make critic
                                                                                               // =2*viewer weight , etc
                user.getUserLevel());
    ...
    private void addReviewToXMap(Map<String, List<reviewModel>> xMap, String key, reviewModel review) {
    ...
    public List<String> getTopNMoviesByCriticsInGenre(Integer N, String userLevel, String genre) {
        // userLevel="critic";
        // userModel user = userService.getUserFromName(userName);
        ...
        List<reviewModel> topReviews = movies.stream().filter(review -> userLevel.equals(review.getUserLevel())) // -----
                .sorted((a, b) -> b.getRating().compareTo(a.getRating())).limit(N).collect(Collectors.toList());
        return topReviews.stream().map(review -> review.getMovieName()).collect(Collectors.toList());
    }

    private List<reviewModel> getReviewByMovieNames(List<movieModel> movies) {
        return movies.stream().map(movie -> movieReviews.get(movie.getName())).filter(Objects::nonNull)
